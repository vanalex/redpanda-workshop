...
pub async fn produce_one(producer: &FutureProducer, schema: &Schema, topic: &str, i: usize) -> Result<()> {
    ...
    let value = to_value(notification)?;
    let payload = to_avro_datum(schema, value)?;
